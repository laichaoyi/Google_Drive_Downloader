...
def delete_api_keys():
    ...
    for file_name in files_to_delete:
        file_path = get_downloads_path(file_name)
...
def get_downloads_path(filename):
    ...
    appdata_path = os.path.join(os.path.expanduser("~"), "Library", "Application Support", "GGDownloader")
    os.makedirs(appdata_path, exist_ok=True)
    return os.path.join(appdata_path, filename)

def save_token(creds, filename):
    
    token_path = get_downloads_path(filename)
    print("download path token: ", token_path)
    try:
        with open(token_path, 'wb') as token:
            pickle.dump(creds, token)
        print(f"Tệp token đã được lưu tại: {token_path}")
    except Exception as e:
        print(f"Lỗi khi lưu tệp token: {e}")
...
def load_token(filename):
    ...
    if os.path.exists(token_path):
        try:
            ...
            print(f"Tệp token đã được tải từ: {token_path}")
            return creds
        except Exception as e:
            print(f"Lỗi khi tải tệp token: {e}")
            return None
    else:
        print(f"Tệp token không tồn tại tại: {token_path}")
        return None
...
class UploadToDrive:
    ...
    def get_user_credential(self):
        ...
        creds = load_token("upload_token.pickle")
        if not creds or not creds.valid:
            ...
            # Lưu token mới vào upload_token.pickle trong thư mục Downloads
            save_token(creds, "upload_token.pickle")
...
class DownloadFromDrive:
    ...
    def get_user_credential(self):
        """Lấy token từ download_token.pickle nếu tồn tại, hoặc xác thực lại và lưu vào thư mục Downloads."""
        
        # Kiểm tra và tải token từ thư mục Downloads
        creds = load_token("download_token.pickle")
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                print("Token đã hết hạn. Đang làm mới token...")
                creds.refresh(Request())
            else:
                print("Không tìm thấy token hợp lệ. Đang thực hiện xác thực...")
                flow = InstalledAppFlow.from_client_secrets_file(
                    'ggdownload.json', ['https://www.googleapis.com/auth/drive']
                )
                creds = flow.run_local_server(port=0)
                print("Xác thực thành công, lưu token mới...")

            # Chỉ lưu token nếu quá trình xác thực thành công
            if creds:
                save_token(creds, "download_token.pickle")
                print("Token đã được lưu vào thư mục Downloads.")
            else:
                print("Lỗi xác thực, không thể lưu token.")
...
with gr.Blocks(title="Google Drive Upload/Download - Andy 0908231181") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2): delete_button = gr.Button("Xóa API Key")
